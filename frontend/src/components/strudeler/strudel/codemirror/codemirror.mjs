...
import { flash, isFlashEnabled } from "@strudel/codemirror/flash.mjs";
import {
  highlightMiniLocations,
  isPatternHighlightingEnabled,
  updateMiniLocations,
} from "./highlight.mjs";
import { keybindings } from "@strudel/codemirror/keybindings.mjs";
import {
  initTheme,
  activateTheme,
  theme,
} from "@strudel/codemirror/themes.mjs";
import {
  sliderPlugin,
  updateSliderWidgets,
...
import { commonCodeCharCountField } from "./commonCodeCharCount.mjs";

const extensions = {
  isLineWrappingEnabled: (on) => (on ? EditorView.lineWrapping : []),
  isBracketMatchingEnabled: (on) =>
    on ? bracketMatching({ brackets: "()[]{}<>" }) : [],
  isBracketClosingEnabled: (on) => (on ? closeBrackets() : []),
  isLineNumbersDisplayed: (on) => (on ? lineNumbers() : []),
  theme,
  isPatternHighlightingEnabled,
  isActiveLineHighlighted: (on) =>
    on ? [highlightActiveLine(), highlightActiveLineGutter()] : [],
  isFlashEnabled,
  keybindings,
};
const compartments = Object.fromEntries(
  Object.keys(extensions).map((key) => [key, new Compartment()])
);
...
export const codemirrorSettings = persistentAtom(
  "codemirror-settings",
  defaultSettings,
  {
...
export class StrudelMirror {
  constructor(options) {
    ...
    this.repl = repl({
      ...
      afterEval: (options) => {
        ...
        const drawTime = options.pattern.getPainters().length
          ? this.drawTime
          : [0, 0];
  ...
  draw(haps, time, painters) {
    painters?.forEach((painter) =>
      painter(this.drawContext, time, haps, this.drawTime)
    );
  }
